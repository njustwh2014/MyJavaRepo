...
### 2.2.2 final局部变量 ###
final局部变量由程序员进行**显式初始化**，如果final局部变量已经进行了初始化则后面就不能再次进行更改，如果final变量未进行初始化，可以进行赋值，**当且仅有一次**赋值，一旦赋值之后再次赋值就会出错。下面用具体的代码演示final局部变量的情况：
...
```
可以**看出String的value就是final修饰的**，上述其他几条性质也是吻合的。
